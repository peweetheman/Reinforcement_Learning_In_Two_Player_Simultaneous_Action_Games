...
class CNN_NashDQN(nn.Module):
    def __init__(self, num_inputs):
        super(CNN_NashDQN, self).__init__()

        self.num_inputs   = num_inputs                  # concatenation of context vector and then state vector
        self.num_actions = num_actions
        self.num_outputs  = num_actions * num_actions
        self.input_shape = env.observation_space.shape

        self.features = nn.Sequential(
            nn.Conv2d(in_channels=self.input_shape[2], out_channels=32, kernel_size=8, stride=4),
            nn.ReLU(),
        ...
        self.noisy_value1 = NoisyLinear(self.feature_size() + GRU_hidden_size, 512, use_cuda=USE_CUDA)
        self.noisy_value2 = NoisyLinear(512, 1, use_cuda=USE_CUDA)

        self.noisy_advantage1 = NoisyLinear(self.feature_size() + GRU_hidden_size, 512, use_cuda=USE_CUDA)
        self.noisy_advantage2 = NoisyLinear(512, self.num_outputs, use_cuda=USE_CUDA)

    def forward(self, state, context):           #  context and state
        x = state.permute(0, 3, 1, 2)            # input of shape NCHW
        batch_size = x.size(0)
        ...
        if batch_size > 1:
            ...
            context = context.squeeze().unsqueeze(0)
        value = F.relu(self.noisy_value1(torch.cat((x, context), dim=1)))
        value = self.noisy_value2(value)
        
        advantage = F.relu(self.noisy_advantage1(torch.cat((x, context), dim=1)))
    ...
    def feature_size(self):
        return self.features(autograd.Variable(torch.zeros(1, *self.input_shape)).permute(0, 3, 1, 2)).view(1, -1).size(1)

    def act_single_action(self, state, context):
        with torch.no_grad():
            state = Variable(torch.cuda.FloatTensor(state)).unsqueeze(0)
            context = Variable(torch.cuda.FloatTensor(context)).unsqueeze(0)
            q_values = self.forward(state, context).data.cpu()
...
current_model = CNN_NashDQN(2 + GRU_hidden_size)
target_model  = CNN_NashDQN(2 + GRU_hidden_size)

context_GRU = nn.GRU(input_size=np.prod(env.observation_space.shape)+1, hidden_size=GRU_hidden_size, num_layers=num_GRU_layers) # inputs are (state, opponent_action) tuples
...
for frame_idx in range(1, num_frames + 1):
    a_t = current_model.act(s_t, c_tm1)
    ...
    action1, action2 = a_t
    input = np.append(s_t.flatten(), action2)
...
with open('2_player_pong.txt', 'w') as f:
