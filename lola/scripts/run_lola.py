...
@click.option("--trace_length", type=int, default=None,
              help="Length of the traces.")
@click.option("--exact/--no-exact", default=False,
              help="Whether to run the exact version of lola.lola.")
@click.option("--pseudo/--no-pseudo", default=False,
              help="Only used with exact version of lola.lola.")
...
@click.option("--opp_model/--no-opp_model", default=True,
...
def main(exp_name, num_episodes, trace_length, exact, pseudo, grid_size,
    ...
    if exact:
        ...
        def run(env):
            from lola.lola.train_exact import train
            train(env,
        ...
        def run(env):
            from lola.lola.train_pg import train
            train(env,
                  num_episodes=num_episodes,
                  trace_length=trace_length,
        ...
        def run(env):
            from lola.lola.train_cg import train
            train(env,
                  num_episodes=num_episodes,
                  trace_length=trace_length,
                  batch_size=batch_size,
